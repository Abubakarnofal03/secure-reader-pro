...
export function ContentUpload({ onSuccess }: ContentUploadProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // Send push notification to all users about new content
      try {
        ...
        const fcmTokens = tokensData?.map(p => p.fcm_token).filter(Boolean) || [];
        ...
        if (fcmTokens.length > 0) {
          ...
          console.log('Push notification sent for new  content');
        }
      } catch (notifError) {
